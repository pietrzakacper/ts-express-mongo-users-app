...
const UserDTO = {
    fromDocument: (doc: UserDocument) =>
        userDTOSchema.parseAsync({
...
export class UsersController {
    ...
    async list(sortByCreated: 'asc' | 'desc'): Promise<UserDTO[]> {
        ...
        const ascByCreated = (a: UserDTO, b: UserDTO) =>
            a.created_at.getTime() - b.created_at.getTime();
        const descByCreated = (a: UserDTO, b: UserDTO) =>
...
function isEmailInUseError(err: any): boolean {
    return (
        ...
        typeof err.errorResponse?.keyPattern?.email === 'number'
    );
}
