...
describe('/users', () => {
    ...
    test('GET /users returns created users', async () => {
        const user1CreatedResponse = await postCreateUser({name: "John Doe", email: "john@gmail.com"})
    ...
    test('GET /users returns users sorted by creation date', async () => {
        const responses = await Promise.all([
            ...
            sleep(1000).then(() => postCreateUser({name: "Mark Idaho", email: "mark@hotmail.com"})),
        ])
